...
class Player(QLabel):
    ...
    def __init__(self, h, w):
        super().__init__()
        self.setPixmap(QPixmap("Images/rocketship.png"))
        self.angle = 90
        self.step = 10
        self.win_h = h
        self.win_w = w
        ...
        self.fire.connect(self.fire_bullet)
        self.timer = QBasicTimer()
        self.timer.start(30, self)
        self.pushedBtn = False  # ako je korisnik pritisnuo dugme za pokretanje rakete
        self.moveX = float(0)
        self.moveY = float(1)
        self.xFull = float(0)
        self.yFull = float(250)
        self.meci = []
    ...
    def move_left(self):
        ...
        global paused
        i = (i + 1) % 72
        self.setPixmap(QPixmap(rocketsList[i]))
        self.angle = self.angle + 5
        print(self.angle)
        self.moveX = cos(radians(self.angle))
        self.moveY = sin(radians(self.angle))
    ...
    def infiniteMoveShip(self):
        ...
        self.move(self.xFull, self.yFull)
        print(self.xFull, self.yFull)
        if (math.floor(self.yFull) <= -255):
            self.yFull = (self.yFull * -1) - 1.0
        elif (math.floor(self.yFull) >= 255):
            self.yFull = (self.yFull * -1) + 1.0
        elif (math.floor(self.xFull) <= -22):
            self.xFull = 559
        elif (math.floor(self.xFull) >= 560):
    ...
    def fire_bullet(self):
        metak = Bullet(self.x(), self.y(), self.angle, i)
        self.meci.append(metak)

    def timerEvent(self, a0: 'QTimerEvent'):
        if (self.pushedBtn == True):
            self.infiniteMoveShip()
        if len(self.meci) > 0:
            for metak in self.meci:
                  metak.kreni.emit()
